...
- **README synchronization**: Every time a new core feature, technical optimization, or architectural change is implemented, the `README.md` **MUST** be updated to reflect the current state of the engine.
- **History Tracking**: All significant technical changes must be recorded in `HISTORY.md`. Refer to this file for technical context before proposing architectural shifts.
